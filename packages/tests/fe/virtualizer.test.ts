...
describe('Virtualizer', () => {
  it('应该根据容器尺寸与偏移计算可见项并应用缓冲', () => {
    const virtualizer = new Virtualizer({ length: 20, buffer: 2, estimateSize: () => 10 })

    virtualizer.update({ containerSize: 50, offsetSize: 0 })
    expect(virtualizer.getTotalSize()).toBe(200)
    expect(virtualizer.getVisibleItems().map((item) => item.index)).toEqual([0, 1, 2, 3, 4, 5, 6])
  ...
  it('应该在更新单项尺寸时重算总高度与起始位置缓存', () => {
    const virtualizer = new Virtualizer({ length: 5, buffer: 1, estimateSize: (i) => (i + 1) * 10 })
    ...
    const third = virtualizer.getVisibleItems().find((item) => item.index === 3)!
    expect(third.start).toBe(10 + 20 + 60)
  })

  it('reset 应该清除缓存并恢复偏移', () => {
    const virtualizer = new Virtualizer({ length: 3, buffer: 0, estimateSize: () => 20 })
