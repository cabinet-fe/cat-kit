...
describe('Validator', () => {
  it('object.safeParse 应该返回 success: true 与数据', () => {
    const user = vObject({ id: vNumber(), name: vString(), active: vBoolean(), createdAt: vDate() })

    const input = { id: 1, name: 'cat', active: true, createdAt: new Date('2024-01-01') }
  ...
  it('object.safeParse 应该收集字段错误并返回路径', () => {
    const user = vObject({ id: vNumber(), name: vString() })
  ...
  it('optional 应该允许字段缺省并返回 undefined', () => {
    const v = vObject({ age: vOptional(vNumber()) })
  ...
  it('optional 默认值应该在 undefined 时生效', () => {
    const v = vObject({ age: vOptional(vNumber(), { default: 18 }) })
