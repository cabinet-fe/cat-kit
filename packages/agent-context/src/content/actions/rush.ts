...
- 描述为空 → 向用户获取描述后继续执行。
- 描述仍存在范围边界、技术路径或验收标准歧义 → 通过 ${target.askToolName} 提供选项：1) 转为 plan 协议分步规划（推荐） 2) 补充描述后重新尝试 rush 3) 终止操作，按用户选择执行。
- 存在未归档的已执行当前计划 → 通过 ${target.askToolName} 提供选项：1) 运行 \`agent-context done\` 归档后继续（推荐） 2) 终止操作，按用户选择执行。
- 存在未实施的当前计划 → 通过 ${target.askToolName} 提供选项：1) 先执行 implement 实施当前计划 2) 运行 \`agent-context done\` 归档后继续 rush 3) 终止操作，按用户选择执行。
