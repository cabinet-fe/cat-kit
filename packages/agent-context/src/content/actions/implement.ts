...
- 带描述 → 通过 AskUserQuestion 提供选项：1) 忽略描述，直接实施当前计划（推荐） 2) 转为 patch 协议处理 3) 终止操作，按用户选择执行。
- 当前计划不存在 → 通过 AskUserQuestion 提供选项：1) 创建新计划 2) 终止操作，按用户选择执行。
- 当前计划状态为 \`已执行\` → 通过 AskUserQuestion 提供选项：1) 通过 patch 追加变更 2) 运行 \`agent-context done\` 归档后创建新计划 3) 终止操作，按用户选择执行。
