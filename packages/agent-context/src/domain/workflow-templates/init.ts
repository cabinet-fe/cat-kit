...
2. **处理 AGENTS.md**：
   - 新项目：
     ...
     2. 生成高质量 ${code('AGENTS.md')}（须满足下方质量标准）。
     3. 继续执行 ${code(c.cmd('plan'))} 创建初始计划。
   - 旧项目：
     1. 若根目录不存在 ${code('AGENTS.md')}：通过提问与代码阅读收集信息后生成。
     2. 若已存在 ${code('AGENTS.md')}：按质量标准评估，不足时增补优化。
     3. 默认不创建计划（除非用户明确要求）。

## 高质量 AGENTS.md 标准

生成或优化 ${code('AGENTS.md')} 时必须满足以下检查清单：
...
- 完成后向用户反馈：项目类型判定结果、AGENTS.md 处理结果、是否需要后续 ${code(c.cmd('plan'))}。
