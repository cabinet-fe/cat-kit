...
`
    ...
    const output = await transform!(source, '/repo/docs/content/demo.md')

    expect(output).toContain(`import ClientBrowserDebug from '../examples/http/client-browser-debug.vue'`)
...
`
    ...
    expect(output).toContain(`import PluginBrowserDebug from '../examples/http/plugin-browser-debug.vue'`)
    expect(output).toContain(`</script>\n# Demo`)
  })
})
