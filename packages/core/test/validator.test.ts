...
describe('Validator', () => {
  it('object.safeParse 应该返回 success: true 与数据', () => {
    const user = object({ id: vNumber(), name: vString(), active: vBoolean(), createdAt: vDate() })
  ...
  it('object.safeParse 应该收集字段错误并返回路径', () => {
    const user = object({ id: vNumber(), name: vString() })
  ...
  it('optional 应该允许字段缺省并返回 undefined', () => {
    const v = object({ age: optional(vNumber()) })
  ...
  it('optional 默认值应该在 undefined 时生效', () => {
    const v = object({ age: optional(vNumber(), { default: 18 }) })
