...
export function union(...arrList) {
    return Array.from(new Set(arrList.flat()));
}
/**
 * 合并多个对象数组，并指定去重字段
 ...
 * @param arrList 任意多个数组
 */
export function unionBy(key, ...arrList) {
    let s = new Set();
 ...
 * @param cb 回调
 */
export function eachRight(arr, cb) {
    let len = arr.length;
    while (--len > -1) {
        cb(arr[len], len, arr);
    }
 ...
 */
export function omitArr(arr, indexes) {
...
class Arr {
     ...
     */
    find(condition) {
        return this._source.find(item => Object.keys(condition).every(key => item[key] === condition[key]));
    }
    /** 最后一个元素 */
    get last() {
