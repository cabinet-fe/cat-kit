...
export class Virtualizer {
  ...
  private applyOptions(o: VirtualizerOptions, initial: boolean): void {
    ...
    if (o.buffer !== undefined) this.buffer = Math.max(0, Math.trunc(o.buffer))
  ...
  private recompute(): void {
    ...
    const itemStartIndex = range ? Math.max(0, range.startIndex - this.buffer) : -1
    const itemEndIndex = range ? Math.min(this.count - 1, range.endIndex + this.buffer) : -1
