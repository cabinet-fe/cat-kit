...
| agent、workflow、plan、replan、implement、patch、done、CLI | `@cat-kit/agent-context` | [Agent Context](/packages/agent-context/) | [Action 说明](/packages/agent-context/actions) / [AI 协作场景](/packages/agent-context/collaboration) / [CLI 命令](/packages/agent-context/cli) | `agent-context workflow plan patch cli` |
...
- 先根据运行环境筛选：
  - Browser：优先看 `http`、`fe`
  - Node.js / Bun：优先看 `be`、`maintenance`、`agent-context`
