...
await createGitTag({ cwd: '/path/to/repo', tag: 'v1.2.3', message: '发布 1.2.3', push: true })

// 强制覆盖
await createGitTag({ cwd: '/path/to/repo', tag: 'v1.2.3', force: true, push: true })
```
...
// 带 2FA
await publishPackage({ cwd: '/path/to/pkg', otp: '123456' })

// Dry run
await publishPackage({ cwd: '/path/to/pkg', dryRun: true })
...
// 全部工作区发布
await publishPackage({ cwd: '/path/to/monorepo', workspaces: true })
```
...
// 1. 提交并推送
await commitAndPush({ cwd, message: 'chore: release v1.2.3' })
