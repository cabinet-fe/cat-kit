...
- 只在**命中客观歧义判据**时做澄清（描述极短 / 含开放性词 / 有多个显著技术路径 / 无验收标准等），避免为走流程而反复追问
- 创建 `plan.md`，包含目标、内容、影响范围和历史补丁四个部分（模板见同步的 `protocols/plan.md`）
- 复杂任务可拆成「一个当前计划 + 多个 `preparing/` 计划」；多计划时最小编号为当前计划，其余入队
- 自检通过前须消除 `## 内容` 中的模糊指令；**只有当步骤 ≥ 6 条、跨 ≥ 2 package、或包含架构 / 选型决策时**才追问是否立即 `review`
...
- 不接受附加需求，遇阻塞向用户报告
- **仅当影响文件 ≥ 5 个、跨 ≥ 2 package、涉及对外 API / 数据库迁移 / 安全敏感逻辑，或实施中做了计划外的显著决策时**，才追问是否执行 `review`；其余情况静默结束
...
- **默认继续 patch**，不再主动判断"关联性"；仅当用户明确表达"这是新需求 / 与之前无关"时，才确认是否归档并新建计划
- **仅当补丁影响 ≥ 5 个文件、涉及对外 API / 数据库 / 安全敏感逻辑，或是 bugfix 类补丁时**才追问 `review`
...
- 仅在描述命中 `plan` 协议的**客观歧义条件**时才澄清；**rush 场景下禁止触发强制反向追问**——如果代理觉得必须反向追问，说明任务本不适合 rush，应转走 `plan`
- 必须做「无模糊指令检查」，有歧义时向用户提问澄清，不得硬 rush
...
- 实施结束后**静默结束**，仅当命中 `implement` 协议的复杂度阈值时才追问 `review`
- 不适合需求范围模糊的大任务
...
- 若 host 支持 subagent capability 且用户明确同意，可用独立子代理审查；否则在当前会话中按同一清单审查
